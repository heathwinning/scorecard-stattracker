-- Full, idempotent refresh of the system scorecard catalog.
-- Uses upserts so existing game sessions, their snapshots, and entered values remain intact.
-- User-created templates are not touched.
...
-- ============================================================
INSERT INTO games (id, name, slug, category, player_count, icon) VALUES
...
ON CONFLICT(id) DO UPDATE SET name=excluded.name, slug=excluded.slug, category=excluded.category, player_count=excluded.player_count, icon=excluded.icon;
...
ON CONFLICT(id) DO UPDATE SET name=excluded.name, description=excluded.description, game_id=excluded.game_id, is_public=excluded.is_public, created_by=excluded.created_by, updated_at=datetime('now');
...
INSERT INTO template_rule_sets (id, template_id, rule_key, label, help_text, definition_json, sort_order) VALUES
...
ON CONFLICT(id) DO UPDATE SET template_id=excluded.template_id, rule_key=excluded.rule_key, label=excluded.label, help_text=excluded.help_text, definition_json=excluded.definition_json, sort_order=excluded.sort_order;
